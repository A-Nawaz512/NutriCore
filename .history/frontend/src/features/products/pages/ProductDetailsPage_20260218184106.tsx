...
import ProductImg from "../../../assets/About/Hero02.jpg"
...
const ProductDetailsPage: FC = () => {
  ...
  // Product passed via state (if navigating from a list) or null initially
  ...
  // Load product if not available in state (e.g., page refresh or direct link)
  useEffect(() => {
    if (!product && productId) {
      ...
      if (found) {
        ...
        // Optional: redirect after showing error message
  ...
  // Set default capsule quantity when product is loaded
  useEffect(() => {
  ...
  const healthBenefits: string[] = Array.isArray(product.healthBenefits) ? product.healthBenefits : []
  ...
  return (
    <div className="container mx-auto px-6 py-10 grid md:grid-cols-2 gap-10">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <img
          src={images.length > 0 ? images[0] : ProductImg} // fallback to local image
