...
const ProductDetailsPage: FC = () => {
  ...
  const healthBenefits: string[] = product.healthBenefits ?? []
  const ingredients: string[] = product.ingredients ?? []
  const supplementOptions: number[] = product.supplementQuantityOptions ?? []


  console.log("my product ", images);
  ...
  return (
    <div className="container mx-auto px-6 py-10 grid md:grid-cols-2 gap-10">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <img
  src={images.length > 0 ? images[0] : ProductImg} // take first image if available, else fallback
