...
const AdminSidebar = () => {
      ...
      <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
        ...
        {menu.map((item) => (
          <NavLink
            ...
            onClick={() => setMobileOpen(false)}
            className={({ isActive }) =>
              `group flex items-center justify-between px-3 py-2.5 rounded-lg transition-all duration-200 text-sm font-medium ${
                isActive
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar */}
      <div
        className={`md:hidden fixed top-0 left-0 z-50 h-full w-64 shadow-2xl transition-transform duration-300 ease-out ${
